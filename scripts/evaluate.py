...
from data.objectnet_dataset import ObjectNetDataset
from data.robustness_dataset import RobustnessDataset
...
def parse_arguments():
    ...
    parser.add_argument('--lr', '--learning-rate', default=5e-4, type=float, metavar='LR', dest='lr')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    setup_distributed(gpu, ngpus_per_node, args)

    model = build_and_distribute_model(ngpus_per_node, args)
    _ = load_checkpoint(model, args)

    cudnn.benchmark = True

    val_loader = create_validation_loader(args)
...
def setup_distributed(gpu, ngpus_per_node, args):
...
def build_and_distribute_model(ngpus_per_node, args):
...
def create_validation_loader(args):
    if args.isObjectNet:
