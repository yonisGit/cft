...
class VisionTransformer(nn.Module):
    ...
    def relprop(self, cam=None,method="grad", is_ablation=False, start_layer=0, **kwargs):
        ...
        elif method == "grad-rollout":
            attn_cams = []
            grad_cams = []
            for blk in self.blocks:
                grad_heads = blk.attn.get_attn_gradients().clamp(min=0)
                attn_heads = blk.attn.get_attn_cam().clamp(min=0)
                avg_heads = (attn_heads.sum(dim=1) / attn_heads.shape[1]).detach()
                avg_heads_grad = (grad_heads.sum(dim=1) / attn_heads.shape[1]).detach()
                attn_cams.append(avg_heads)
                grad_cams.append(avg_heads_grad)
            cam = compute_rollout_attention(attn_cams, start_layer=start_layer)
            gam = compute_rollout_attention(grad_cams, start_layer=start_layer)
            cam = cam * gam
            cam = cam[:, 0, 1:]
            return cam
